...
export const PlaygroundEditor = ({
  ...
  const acceptCurrentSuggestion = useCallback(() => {
    ...
    try {
      ...
      // Defer parent callback so it doesn't collide with Monaco's own
      // synchronous onChange → setState chain from executeEdits above
      queueMicrotask(() => {
  ...
  const handleEditorDidMount = (editor: any, monaco: Monaco) => {
    ...
    // NOTE: editor.addCommand() returns a command ID string, not a disposable object,
    // so there is nothing to dispose here before reassigning it.
    ...
    editor.onDidChangeModelContent((e: any) => {
      ...
      if (e.changes.length > 0 && !suggestionAcceptedRef.current) {
        ...
        if (
          ...
          }, 500) // Small delay to let the change settle
        }
      }
    })
