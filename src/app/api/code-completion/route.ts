...
async function generateSuggestion(prompt: string): Promise<string> {
  try {
    const response = await fetch("http://localhost:11434/api/generate", {
      ...
      body: JSON.stringify({
        model: "qwen2.5-coder:1.5b", // matches `ollama list` exactly
        prompt,
        stream: false,
        options: {
          temperature: 0.7,
          num_predict: 300, // Ollama uses num_predict, not max_tokens
        },
      }),
    });
    ...
    // Clean up the suggestion (strip markdown code fences if present)
